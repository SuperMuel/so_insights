...
async def handle_search_query_set(
    ...
    logger.info(
        f"Processing search query set {search_query_set.id} ({search_query_set.title}, {len(search_query_set.queries)} queries, timelimit='{settings.TIME_LIMIT}')"
    )
    ...
    if not articles:
        ...
        try:
            ...
            logger.info(f"Inserted {len(inserted.inserted_ids)} articles to mongodb")
